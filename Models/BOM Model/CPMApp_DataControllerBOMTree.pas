...
type
  ...
  TAppMamulKartData = class
    ...
    // Mamül Stok Kart ve Stok Birim Alanlarý
    ...
    // Hammadde Stok Kart, Stok Birim ve Mamül Baþlýk Alanlarý
  ...
  public
    ...
    procedure Expand(MamulData: TAppBOMData); overload;
    procedure Expand(MamulKod, VersiyonNo, KullanimKod: string; SurumNo: Integer; Tarih: TDateTime; Miktar: Double; Birim, MamulDepoKod, HammaddeDepoKod: string; OzelReceteTip, EvrakTip: Smallint; HesapKod, EvrakNo: String; EvrakSiraNo: Integer); overload;
    procedure Expand(MamulKod, VersiyonNo, KullanimKod: string; SurumNo: Integer; Tarih: TDateTime; Miktar: Double; Birim, MamulDepoKod, HammaddeDepoKod: string); overload;
...
begin
  FOwner := AOwner;
                         ...
                         // Aðaç        Üretim       MRP     Açýklama
  SingleLevel := True;   // Parametrik  True         True    Genel
  SipariseUretim := False;
  ReturnPhantom := False;// Parametrik  True         True    Aðaç için eklendi
  ...
  // Stok Kapat kullanýldýðýnda verilmek zorunda.
  FMamulDepoKod := '';
  FHammaddeDepoKod := '';
...
begin
  ...
  try
    ...
    AData.SeviyeKod := 0; // 0 - Kendisi 1- Birinci Seviye ve 2 den sonra diðer seviyeler
...
var
  ...
  begin
    ...
    begin
      if FMamulKart.TableRotaKaynak.KullanimTip = 1 then // Sadece kullanýlým tip = 1 olanlarý ekliyoruz. 30.12.2013 Veysel
      begin
  ...
  begin
    ...
    begin
      ...
      KaynakIslemData.ParentID := ARotaData.ID; // Sadece aðaç gösterimi için
...
//        KaynakData.Kullanilan := FMemTableKaynak.KullanimTip = 1;
      ...
      KaynakData.ParentID := ARotaData.ID; // Sadece aðaç gösterimi için
  ...
  begin
    ...
    begin
      ...
      RotaData.ParentID := AMamulDataID; // Sadece aðaç gösterimi için
  ...
  begin
    ...
    else if HammaddeData.SonSeviye then // Hammadde (Hammadde veya Yarý Mamül) veyahut Yan Ürün
    begin
      ...
      if (HammaddeData.MalTip <> botCoProduct) and (HammaddeData.YanUrunNo <> 0) then // Yan Ürün No tanýmlý bir hammadde ise
      begin
      ...
      DoOnReturn(HammaddeData);
    end else // Assembly (Mamül, Yarý Mamül veya Hayalet Montaj)
  ...
  begin
    ...
    begin
      // Firenin firesini almamasý için önce oraný al sonra sabit miktarý ekle.
    ...
    // Bileþen Firesini Ekle //
    if MamulKartData.BilesenFireOran > 0 then // Mamül Kart tablosuna Bileþen Fire Oraný Girilmiþ Ýse
      Result := Result + (Miktar * MamulKartData.BilesenFireOran / 100)
    else // Hammadde Tablosuna girilmemiþ ise stok kart da yazan bileþen fire oranýný kullan.
  ...
  begin
    ...
    if MamulData.Birim <> MamulKartData.MamulBirim then // Ýstenen mamülün birimi mamül aðacý birimi ile ayný ise çevrime gerek yok.
    begin
      ...
      begin
        if dcCommon.StokBirim.Find(MamulData.MalKod, MamulData.Birim) and (dcCommon.StokBirim.Table.Katsayi > 0) then
          ...
          DoOnReturnError('BOM Explosion', ErrCode_StokBirimCevrimNotFound, format('%s nolu mamülün mamül baþlýðýnda tanýmlý %s biriminin stok kart birimine çevrim kat sayýsý stok birim tablosunda bulunamadý', [MamulData.MalKod, MamulData.Birim]), MamulData);
      end;

      if MamulKartData.MamulBirim <> MamulKartData.MamulStokKartBirim then // Sonra stok birimini mamül aðacý birimine çevir
      begin
        if MamulKartData.MamulStokBirimKatsayi > 0 then
          vMamulMiktar := vMamulMiktar * MamulKartData.MamulStokBirimKatsayi
        else
          DoOnReturnError('BOM Explosion', ErrCode_StokBirimCevrimNotFound, format('%s nolu mamülün mamül baþlýðýnda tanýmlý %s biriminin stok kart birimine çevrim kat sayýsý stok birim tablosunda bulunamadý', [MamulData.MalKod, MamulKartData.MamulBirim]), MamulData);
      end;
    end;
    ...
    begin
      ...
      begin
        ...
        DoOnReturnError('BOM Explosion', ErrCode_StokBirimCevrimNotFound, format('%s nolu hammaddenin %s nolu mamül kartýnda tanýmlý %s biriminin stok kart birimine çevrim kat sayýsý stok birim tablosunda bulunamadý', [MamulKartData.HammaddeKod, MamulData.MalKod, MamulKartData.HammaddeBirim]), MamulData);
    end;

    // Mamülün Hammadde Ýhtiyacýný bul //
    ...
    if Params.SipariseUretim then // Sipariþe üretim ise yarý mamülün üretim yönteminin sipariþe üretim olup olmadýðý önemli
      if not HammaddeData.SonSeviye then
        HammaddeData.SonSeviye := MamulKartData.HammaddeStokKartTeminTip <> Integer(ttOrder); // Sipariþe üretim deðil ise hammadde seviyesidir, daha alta inme
    ...
    if MatchFields.TableDes <> nil then // TableDes verilmiþse match field yap
    begin
    ...
    if Params.LotKapat and (HammaddeData.MalTip <> botPhantomAssembly) and (HammaddeData.MalTip <> botCoProduct) then // Alternatiflerin çalýþabilmesi için lot kapat parametresi true olmak zorunda.
    begin
      ...
      begin
        ...
        if (HammaddeData.MalKod = dcLotKapat.TableLot.MalKod) and (HammaddeData.VersiyonNo = dcLotKapat.TableLot.VersiyonNo) then // Yarý mamülse ve alternatifi kullanýlmýþsa.
          HammaddeData.SurumNo := MamulKartData.HammaddeSurumNo
        else
    ...
    try
      ...
      begin
        ...
        if vOzelMamulKart.Eof then
          DoOnReturnError('BOM Explosion', ErrCode_MamulKartNotFound, format('%s nolu malýn %s nolu versiyonunun özel mamül kartý bulunamadý.', [MamulData.MalKod, MamulData.VersiyonNo]), MamulData)
        else begin
          vHasOzelRecete := True;
          for I := 0 to MatchFields.FieldSrcList.Count - 1 do
      ...
      begin
        ...
        if FMamulKart.TableKart.Eof then
          DoOnReturnError('BOM Explosion', ErrCode_MamulKartNotFound, format('%s nolu malýn %s nolu versiyonunun mamül kartý bulunamadý.', [MamulData.MalKod, MamulData.VersiyonNo]), MamulData)
        else
        begin
          for I := 0 to MatchFields.FieldSrcList.Count - 1 do
...
var
  ...
  begin
    ...
    if FMamulKart.TableKart.HammaddeBirim <> FMamulKart.TableKart.HammaddeStokKartBirim then // Mamül aðacý birimi stok biriminden farklý ise mamül aðacý birimine çevrilir.
    begin
      if FMamulKart.TableKart.HammaddeStokBirimKatsayi > 0 then
        ...
        DoOnReturnError('BOM Explosion', ErrCode_StokBirimCevrimNotFound, format('%s nolu mamülün mamül baþlýðýnda tanýmlý %s biriminin stok kart birimine çevrim kat sayýsý stok birim tablosunda bulunamadý', [FMamulKart.TableKart.MamulKod, FMamulKart.TableKart.MamulBirim]), Data);
    end;
    ...
    begin
      if FMamulKart.TableKart.MamulStokBirimKatsayi > 0 then
        ...
        DoOnReturnError('BOM Explosion', ErrCode_StokBirimCevrimNotFound, format('%s nolu hammaddenin %s nolu mamül kartýnda tanýmlý %s biriminin stok kart birimine çevrim kat sayýsý stok birim tablosunda bulunamadý', [FMamulKart.TableKart.HammaddeKod, FMamulKart.TableKart.MamulKod, FMamulKart.TableKart.HammaddeBirim]), Data);
    end;
    ...
    // Bileþen Firesini Ekle //
    if FMamulKart.TableKart.BilesenFireOran > 0 then // Mamül Kart tablosuna Bileþen Fire Oraný Girilmiþ Ýse
...
begin
  ...
  try
    ...
    try
      ...
      FMamulKart.TableKart.First;
      if FMamulKart.TableKart.Eof then
        DoOnReturnError('BOM Explosion', ErrCode_MamulKartNotFound, format('%s nolu malýn %s nolu versiyonunun mamül kartý bulunamadý.', [Data.MalKod, Data.VersiyonNo]), Data);
      while not FMamulKart.TableKart.Eof do
      begin
        AddChild;
        FMamulKart.TableKart.Next;
